...
bool zaznobin_p_interg_method_of_rectangles_mpi::TestMPITaskSequential::validation() {
  ...
  double validation_a = *reinterpret_cast<double*>(taskData->inputs[0]);
  double validation_b = *reinterpret_cast<double*>(taskData->inputs[1]);
...
bool zaznobin_p_interg_method_of_rectangles_mpi::TestMPITaskParallel::validation() {
  ...
  if (world.rank() == 0) {
    ...
    int validation_n = *reinterpret_cast<int*>(taskData->inputs[2]);
    if (validation_n <= 0) {
      return false;
    }
  }
